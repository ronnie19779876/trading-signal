...
- 修复：**盈透网关每日重启后，实时账户整个白天取不到资金**（2026-09-21 生产实测）。网关美东 03:45 重启、17 秒后重连，
  盈透侧上一个会话的账户汇总订阅仍占着"每客户端 2 个"的名额，新订阅被 322 拒；`money` 一直为空，实时账户卡在 WARMING，
  持仓与 reqPnL 不受影响。人工断开重连即恢复。三处改动：
  - 换会话重订前，按记下的上一个汇总请求 id 先补发一次取消（`IbkrLiveAccount.staleSummaryId`）；持仓与账户盈亏没有这个现象，仍不补发。
