...
| `reqPnL` **首条不对**（新会话只含一只持仓；同一会话退订再订给出的也不对，09-14 / 09-19 三次实测）；价格静止时第二条可能一直不来（生产 09-18 盘后 20 分钟）；`reqPnLSingle` 的 realized 恒为 Double.MAX_VALUE | 丢首条 + 10 秒看门狗重订（最多 3 次），不要用逐只加总顶替；MAX / NaN / 无穷一律映射成 null。1101 重订前先取消同一会话的旧订阅（`IbkrLiveAccount`） |
...
**生产跑 3.0.3**（2026-09-18 美东 21:10 部署；3.0.2 于 20:36 部署、含未单独部署的 3.0.1；第 3 期见 ARCHITECTURE §16，2.0.2 全量审查后的修复见 §17）。
