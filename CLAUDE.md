...
| `check-secrets.sh` 到 3.1.1 为止还有**两个静默漏报**（2026-09-25 全项目审查发现，探针实证）：① 关键词模式全小写而 `grep` 不带 `-i`，本项目实际使用的 `TRADER_DB_PASSWORD` / `OPENAI_API_KEY` / `TRADER_ACCOUNT_KEY_SECRET` 以及 `.run/*.run.xml` 的 `<env name=… value=…>` 形式**全部漏过**；② IPv4 命中按「整行含回环就跳过整行」豁免，而隧道命令行里回环与真实主机地址同在一行，**真实公网地址被连带放行**。5 行探针只抓到 1 行 | 扫描加 `-i`；取值改为必须以字母数字开头（挡掉 `${VAR:+…}`、`${VAR:}` 这类占位符引用，否则 -i 会带出误报）；另加一条属性形式模式；IPv4 改成**逐个地址判定**。**改扫描规则后必须用探针文件逐条反证**——这次反证时我的第一次删除操作静默失败、结果看上去「仍然 5/5 通过」，差点把没生效的修改当成生效。反证要确认改动真的落到了文件上 |
...
| 盈透账户汇总的「每客户端 2 个」上限算的是**订过的次数**，不是当前活着的订阅：`cancelAccountSummary` 发出去了**名额也不释放**，只有客户端重连才清零（2026-09-23 生产实测，时间线逐格吻合：22:04 实时账户闲置退订占 1 个 → 次日 06:00 快照作业订到第 2 个、作业本身成功 → 06:53 实时账户再订就是第 3 个 → 322，此后每次再订都被拒，直到人工断开重连）。**3.0.8 的判断是错的**：当时以为补发取消能解决，其实部署重启清空了名额才让症状消失；补发取消只记得实时账户自己上次的 reqId，取消不掉快照作业留下的那一个，重试 3 次 × 3 秒对不会释放的名额也毫无意义 | **别再用「用完就退」**。账户汇总改成连接周期内只订一次的常驻订阅（`IbkrAccountSummaryFeed`，3.0.9），实时账户与每日快照共用；闲置退订只摘监听器不退订阅；快照取常驻订阅的最新值（约 3 分钟一批，超过 10 分钟算陈旧，没有就等 90 秒，等不到报错让补偿重试）。**这是规避不是修复**：盈透为什么不释放名额仍然未知；1101 后的重订仍会再占一个名额，待实测 |
...
| 账户当日盈亏**含已平仓标的的当日已实现**，逐只之和只覆盖在持的：**当天只要有平仓，`verifyPnl` 就必然判不通过**，当日盈亏整天空着（2026-09-23 卖出 TSLA 后实测；3.0.3 起就有这个洞，之前没一天平仓所以没暴露） | 核对要扣掉账户级已实现：`|账户当日 − 已实现 − Σ逐只当日| ≤ 1`（3.0.10）。逐只的 realized 恒为 MAX 不可用，账户级的可用。**核对不通过必须把数字记进日志**——09-23 那次只记了「没有有效推送」、一个数字没留，只能靠净值反推去猜口径 |
...
| `mvn test` 与 `mvn verify` 的 <b>classpath 不一样</b>：reactor 在 package 之后给下游模块的是 <b>jar</b> 而不是 `target/classes` 目录。`SpringBeanConstructorTest` 的多模块扫描从 classpath 挑目录，于是 test 下扫到九个模块、verify 下只剩一个——本地全绿、发布前跑 verify 才炸（3.1.2 发布时实际发生） | 要遍历「本仓库各模块的编译产物」就按仓库布局直接找目录，别从 classpath 挑。另：**发布前必须跑 `./mvnw clean verify`，不能只跑 `test`**。还有一条顺带的：javadoc 里别写 `<仓库>/*/target`——注释里的 `*` 加 `/` 就是块注释结束符，会把文件截断 |
...
**生产跑 3.1.2**（2026-09-25 美东 02:16 部署；3.1.1 于 09-24 19:07 部署、3.1.0 于同日 18:59 部署、3.0.10 于 18:15 部署，3.0.9 于 09-23 01:31 部署，3.0.8 于 09-21 18:00 部署、3.0.7 于 09-19 20:01 部署，3.0.6 于 09-19 01:38 部署，审核期间 09-18 晚至 09-19 凌晨先后部署 rc.1～rc.7；第 3 期见 ARCHITECTURE §16，2.0.2 全量审查后的修复见 §17）。
...
**3.0.8 已在生产（定性有误，见 3.0.9）**：当时以为「订阅前补发取消 + 322 限次重订」能修好账户汇总被拒，实际是部署重启清空了名额才让症状消失。
**3.0.9 已在生产（ARCHITECTURE §20.7）**：账户汇总改为**整个连接周期只订一次的常驻订阅**，实时账户与每日快照共用；盈透的「每客户端 2 个」算的是订过的次数、取消不释放名额，只有重连才清零。快照资金口径随之从「当场查」变成「最近一次推送」（约 3 分钟一批）。**验证标准：部署后正常不作数**——必须跨过一次 03:45 网关重启、一次每日快照、一次「闲置退订 → 再读」，并连续 2 个交易日无 322。
...
**3.1.2 已在生产**：全项目审查清单清空——30 项逐条修完（守护测试失效 7、账户与信号 3、存储与作业 6、网关并发 2、AI 与估值 4、REST 与前端 5、运维文档 1，另 1 项证伪），
...
**待真实数据验证的两条**：① 1101 后的账户汇总重订（生产至今没发生过 1101）；
② 3.0.10 的「账户当日盈亏扣除已实现」那条路径（要等下一个有平仓的交易日）。
