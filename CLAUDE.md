...
| 盈透**网关每日重启（美东 03:45）重连后**，旧会话的账户汇总订阅仍占着名额：新订阅被 322 拒，`money` 取不到、实时账户卡在 WARMING 一整天（2026-09-21 实测，持仓与 reqPnL 正常，人工断开重连即好） | 换会话重订前按上一个请求 id 补发一次取消；收到 322 自动退订重订（3 次）；错误恢复后清掉 `lastError`。见 `IbkrLiveAccount`，3.0.8 修 |
