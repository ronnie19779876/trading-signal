...
| 盈透账户汇总订阅**每个客户端最多 2 个**（第 3 个报 322，另一个客户端不受影响，2026-09-19 实测）；常驻订阅之后**逐条推、没有 End、约 3 分钟一批** | 实时账户常驻 1 个 + 每日快照一次性 1 个 = 2，别再加第三处；汇总逐条攒批再发。净值要秒级就用「现金 + 应计股息 + Σ reqPnLSingle.value」估（实测与汇总精确相等） |
| `reqPnL` **首条不完整**（只含一只持仓，1 秒后第二条才完整，09-14 与 09-19 两次实测）；`reqPnLSingle` 的 realized 恒为 Double.MAX_VALUE | 丢首条；MAX / NaN / 无穷一律映射成 null。1101 重订前先取消同一会话的旧订阅（`IbkrLiveAccount`） |
...
**3.0.2 已发布，待部署到生产（ARCHITECTURE §19、§20）**：含 3.0.1（AI 额度按角色、账本 AI 分组、手工分析按钮、前端第 1、2 期）与仪表盘改版、实时账户（`GET /api/account/live`，盈透按需订阅）；无迁移、无配置变更。
