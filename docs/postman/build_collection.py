...
ENDPOINTS = [
    ...
    },
    {
        ...
        "items": [
            ...
            {"name": "幽灵 K 线订正（先试跑）", "method": "POST", "path": "/api/bars/cleanup/phantom", "query": [{"key": "apply", "value": "false"}],
             "desc": "列出落在交易日历之外的 K 线（券商在美股假日给过脏数据：成交额 0、价格离谱）。apply=false 只试跑，确认清单无误后再改 true 真删。"
                     "只删试跑列出来的那些，一次最多 200 条：found 是总数、listed（= bars 长度）是本次列出也是本次最多会删的条数、"
                     "deleted 只可能是 0 或 listed、remaining 是删完还剩多少；found > listed 时再跑一轮。",
             "tests": T_200 + T_JSON + ['pm.test("试跑不应删除", () => { pm.expect(body.applied).to.eql(false); pm.expect(body.deleted).to.eql(0); });',
                                        'pm.test("三个数字自洽", () => { pm.expect(body.listed).to.eql(body.bars.length); '
                                        'pm.expect(body.found).to.be.at.least(body.listed); pm.expect(body.remaining).to.eql(body.found - body.deleted); });']},
            ...
            {"name": "缺口深扫", "method": "GET", "path": "/api/bars/gaps", "query": [{"key": "limit", "value": "20"}],
             "desc": "对照交易日历找缺失的交易日。已知：NBIS 因停牌缺 664 天、SPY 券商缺数 26 天，都补不回来。",
    ...
    },
    {
        ...
        "items": [
            ...
            {"name": "全市场估值（筛选表）", "method": "GET", "path": "/api/fundamentals/valuations",
             "desc": "某一天全部标的的估值，date 缺省取最新有估值的一天；rows 与单只估值同字段。名称、行业、指数从 /api/universe 合并。",
             "tests": T_200 + T_JSON + ['pm.test("有 rows 数组", () => pm.expect(body.rows).to.be.an("array"));']},
    ...
    },
    {
        "folder": "入场信号（第 4 期）",
        "items": [
            {"name": "单日四门判定（现场计算）", "method": "GET", "path": "/api/signals/evaluate/{{symbol}}", "query": [{"key": "date", "value": ""}],
             "desc": "判据版本 sentinel-v1。date 缺省取最近收盘落定的交易日；晚于它 409，非交易日 400。返回每道门的判据原文与代入值、支撑区、出场预案、剔除的非交易日 K 线与缺失交易日。只读、不落库。",
             "tests": T_200 + T_JSON + ['pm.test("有判定状态", () => pm.expect(body.evaluation.status).to.be.a("string"));']},
            {"name": "区间回放", "method": "GET", "path": "/api/signals/replay/{{symbol}}", "query": [{"key": "from", "value": "2025-09-01"}, {"key": "to", "value": ""},
            ...
            {"name": "画图用 K 线（折回判定日口径）", "method": "GET", "path": "/api/signals/bars/{{symbol}}",
             "query": [{"key": "asOf", "value": ""}, {"key": "from", "value": ""}, {"key": "to", "value": ""}],
             "desc": "整段结构口径换算后折回 asOf 当天的价格尺度，与当天的信号价位对齐；默认 to = asOf = 收盘落定日、from 往前一年，最长 3 年。",
             "tests": T_200 + T_JSON + ['pm.test("是数组", () => pm.expect(body).to.be.an("array"));']},
            {"name": "预览发给模型的输入（不计费）", "method": "GET", "path": "/api/signals/ai-input/{{symbol}}", "query": [{"key": "date", "value": ""}],
             "desc": "代码算好的技术面、估值、财报、公司简介与四门判据，键序固定。不含持仓与账户信息。当天不予判定 409。",
             "tests": ['pm.test("HTTP 200 或 409（当天不予判定）", () => pm.expect(pm.response.code).to.be.oneOf([200, 409]));']},
