...
| 4 | 入场信号与 AI 分析（**3.0.0 已发布，§18**）：入场哨兵四门判定、纸面跟踪账本、OpenAI 结构化分析（只有否决权）、信号页。富途基本面增量已在第 2 期步骤 3 交付 |
...
## 18. 第 4 期：入场信号（2026-09-17 发布，3.0.0）
...
### 18.7 每日评估、信号与纸面账本（步骤 3）

设计 2026-09-17 经用户认可。

**表（V11）**
- `signal_evaluation`：一只一天一个判据版本一行，重跑覆盖。汇总列每行都有（状态、结果、四门缩写、通过门数、首个未过的门、
  判定时角色、收盘、ATR、RVOL、区底、止损、止损距离、输入指纹）；判定明细 jsonb 只给池与持仓、至少过三门、结果不是 NO_SIGNAL 的存，
  其余用单日判定接口现场重算。开发库实测每天约 110 行带明细（过三门的比预想多），一年约 80MB。
- `entry_signal`：只追加，价位为判定日口径（当天真实可成交价）；状态 NEW → ACKNOWLEDGED / DISMISSED / EXPIRED，另有 VETOED（步骤 4）；
  有效期到判定日后第 2 个交易日，该日评估后 NEW 与 ACKNOWLEDGED 都过期；对过去日期补跑产生的记 `origin=BACKFILL`，统计与实盘分开。
- `signal_track`：纸面账本，两个出场变体 `BASE`（止损 2.0×ATR）与 `STOP_2_5`（只把止损换成 2.5×ATR），都不减半仓；
  未平仓的每天从信号日整段重算（`SignalTrades`，与回放同一入口），不存增量状态，持有期间的拆股与 K 线订正自然体现。

**输入指纹**：窗口内原始 K 线（含被剔除与空 K）+ 窗口内除权的结构性事件因子 + 判据版本的 SHA-256 前 32 位，数值按规范文本（去尾零）。
判定是确定性的，复算指纹不同只可能是数据被重拉改过（步骤 2 实测成交量会上修）；信号详情接口给出 `fingerprintMatches`。

**作业 `SIGNAL_EVALUATION`**（美东 18:10，交易日）：目标 = 全量成分股 ∪ 池与持仓，去掉基准。每只：数据层检查 → 四门 →
边沿（读上一交易日评估行；没有、或当天不予判定时按判定规则现场重算上一交易日四门，AI 信息未知按未否决——直接当"没过"会让连续满足的第二天重复发信号，守护测试反证过）
→ 冷却（距上一条未被否决的信号的交易日数）→ AI（步骤 3 一律按没有结论）。是信号的评估行、信号、两个变体的账本行同一事务写入（`SignalStore`）；
不是信号的评估行按批写。收尾：过期、重算未平仓账本。重跑同一天：评估覆盖，信号唯一键挡住不重复；重算后不再是信号的由审计提示、信号保留。
K 线与因子按 100 只一批取：逐只往返时开发实例经隧道跑一天 4 分 15 秒，批量后 33 秒。

**补偿**（美东 22:00，与 21:00 行情补偿错开）：当天没有评估，或有判为数据过期、但现在库里已有当天 K 线的标的，就重跑。
**审计**（收盘巡检第五段 `GET /api/signals/audit`）：关键项为评估存在（19:00 前只提示）、覆盖完整、未平仓账本算到当天；
提示项为过期跳过超过 2%、缺日与口径失败名单、信号与重算不一致、最近一次作业。`jobs` 健康指标监控该作业，逾期 120 小时。

**开发库验证**（2026-09-17）：对 2026-07-01 ~ 09-02 共 45 个交易日逐日补跑，全部 OK，23355 行评估（45 × 519）、106 条信号（均为 BACKFILL）、
104 条按 T+2 过期；同一天重跑不重复发信号。
- 作业写入的评估对回放接口逐日比对（池与持仓、全部出过信号的标的、随机 40 只，共 142 只 5538 行，从 07-10 起比以避开补跑起点的冷却盲区）：
  状态、四门缩写、结果**全部一致**。
- 纸面账本对回放的纸面交易逐笔比对：212 行里 209 行入场日、入场价、止损、离场日、离场原因、R 全部一致；3 行不同（TROW 08-31、IBKR 08-07 两个变体）
  是比对口径造成的——账本算到补跑的最后一天 09-02，回放算到收盘落定日，而开发库里这两只有 09-11 的 K 线，回放多走出了 09-10、09-11 的离场，
  离场前的入场、止损、+1R、最大浮盈完全相同。

### 18.8 AI 否决（步骤 4）

设计 2026-09-17 经用户认可：AI 只有否决权；只分析当天将成为信号的候选（边沿与冷却之后），在评估作业里调用
（单次 120 秒、作业内 AI 总时长 15 分钟、每日 20 次）；否决 = 立场 BEARISH / AVOID 且把握不是 LOW 且至少 2 条核对通过的看空证据；
拒答、截断、结构非法、调用失败、预算用尽都按没有结论放行；补跑不调 AI（模型的训练数据含判定日之后的信息）；
只在 `trader.ai.signal-veto-enabled=true` 的实例调用（生产开、开发关）；不发持仓数量、成本、账户，也不说是否持有；不开联网搜索；
被否决的信号照样进账本，按裁决分组比较。

**实测**（2026-09-17）
- SDK 4.52.0 的 Responses API 结构化输出可用：`VetoJudgment`（record + 枚举 + 嵌套列表）生成 strict schema，本地校验通过。
- `daily_bar.pe` 是静态市盈率（66 行里 64 行与估值快照的静态值差 1% 以内，与 TTM 只对上 5 行）；由它反推的 EPS 在年报次日跳变
  （MSFT 2021-07-28 → 8.05 = FY2021 稀释 EPS），历史值按当时已知数据计算，可以做 5 年分位。季报是单季口径。
- 输入 JSON 约 8600 字符。两处确定性问题已修：财报白名单原用 `Map.of`（遍历顺序不定）；`BigDecimal` 去尾零后被写成 `2.547E+9`，
  金额改为百万美元普通小数（键名 `*UsdM`）。
- 真实调用 `gpt-5.6-sol`（推理强度 medium，6 次）：

| 调用 | 结果 | 耗时 | 输入 token（缓存命中） | 输出 token（推理） |
| --- | --- | --- | --- | --- |
| HWM 第 1 次 | NEUTRAL / MEDIUM，8 条证据全部核对通过，放行 | 13.8 秒 | 5430（0） | 883（430） |
| DLR（亏损股） | NEUTRAL / MEDIUM，放行 | 15.2 秒 | 5454（2054） | 972（392） |
| HWM 连调 3 次 | 3 次都是 BULLISH / MEDIUM，放行 | 12.2~20.2 秒 | 5430（5427） | 797~1153（305~708） |
| HWM 输出上限 200 | TRUNCATED（`max_output_tokens`），没有把残缺输出当结论 | 7.0 秒 | 5430（5427） | 200（200） |

  **同一份输入立场会摇摆**（HWM 第一次 NEUTRAL，之后三次 BULLISH），印证了否决要设门槛；前缀缓存有效（提示词 + schema 约 2050 token
  跨标的共享，同一输入重跑几乎全部命中）。

**实现（4.2 / 4.3）**
- `trader-ai`：`VetoJudgment`（输出结构）、`Prompts`（`prompt/sentinel-veto-v1.md`）、`EvidenceVerifier`（字段路径 + 数值相对 ±1%，兼容千分位、百分号、全角）、
  `VetoRule`（否决规则）、`VetoClient` 接口与 `OpenAiVetoClient` 实现（`com.openai.*` 只在实现类；core 只认接口，单测才能替身）。
- `trader-core`：`SignalPayloadBuilder`（输入构建，EnumMap + LinkedHashMap 固定键序；财报只取期末早于判定日的，手工分析过去日期时再往前让 45 天，
  因为库里没有财报公布日）、`AiVetoService`（构建输入 → 同一输入已有 OK 结论复用 → 预算检查 → 调用 → 核对 → 规则 → 落库，失败一律 ABSENT）、
  `AiAnalysisFacade`（手工分析、查询、用量）。评估作业只对 `PENDING_AI` 的候选、且 `signal-veto-enabled` 与实盘判定时调用；否决的写
  `BLOCKED_BY_AI` 评估 + `VETOED` 信号（照样建账本），分析行回填 `signal_id`。账本汇总多一个维度：模型裁决（VETO / ALLOW / NONE）。
- V12 `ai_analysis`：每次调用（含预算跳过、输入构建失败）一行；输入哈希按发出去的 JSON 原文算（jsonb 会重排键）。
- 预算：每日调用上限按美东自然日数实际发出的调用（预算跳过、输入构建失败不算）；作业内总时长到了就不再发起新调用。
- 信号审计加提示项 `aiAnalyses`；生产外置配置模板打开 `trader.ai.signal-veto-enabled`，密钥 `OPENAI_API_KEY` 在 `trader.env`。
- 开发实例验证（2026-09-17，手工分析，从补跑信号里挑财务恶化的两只）：
  GM 2026-07-21 → AVOID / HIGH，5 条看空证据全部核对通过（静态市盈率分位 95.77、单季自由现金流 −1836 百万美元、稀释 EPS 同比 −15.82%、
  年度营业利润同比 −77.25%、财报窗口 3~23 天）→ **VETO**，19.4 秒；WBD 2026-07-13 → AVOID / MEDIUM，5 条全部通过
  （自由现金流 −476、同比 −257.62%、归母净利 −2916、有息负债率 99.66%、流动比率 0.73）→ **VETO**，26.2 秒。
  同一请求再发一次直接复用 #1、不再调用（用量只记 2 次）。开发时一处 SQL 文本块首行的空格被当成缩进去掉、与前一段粘连，
  仓储的 SQL 不在单测覆盖里，调用前先把只读接口都跑了一遍。
- 守护测试（均反证过）：否决的候选记 VETOED 与 BLOCKED_BY_AI、AI 关闭或补跑时不调模型、每日上限与作业时长到了不调用、同一输入复用不调用、
  调用失败放行、编造数字的证据不计入否决、否决四个条件任缺其一都放行。

### 18.9 前端信号页（步骤 5）

设计 2026-09-17 经用户认可。路由 `/signals`（单段），导航"信号"。
- 顶部状态条：信号审计（不通过的项逐条列出，关键项标红）、当天评估数 / 新信号 / AI 分析与否决、AI 是否配置密钥、否决开关、今天调用次数（按美东日期）/ 每日上限。
- 四个标签：**今日评估**（四门漏斗、状态与结果分布、评估表，默认池与持仓，点行打开判定详情抽屉）；**信号**（按状态、来源、范围过滤，
  已看过 / 不做带备注二次确认，点行打开信号详情抽屉）；**纸面账本**（按变体 × 来源 × AI 裁决汇总、两变体配对差、明细）；**回放**（逐日判定 + 纸面交易 + K 线标记）。
- 信号详情抽屉：价位、K 线（判定日 / 入场 / 离场标记，止损、+1R、吊灯止损、目标价位线，支撑区上下沿）、AI 第二意见（多空证据两栏，核对不通过标黄；
  没有结论时"手工分析"按钮二次确认，超时 120 秒）、四门判据原文与指纹核对提示、两个变体的账本。
- **K 线口径**：新增 `GET /api/signals/bars/{symbol}`，整段结构换算后折回判定日价格尺度（`SignalTrades.scaledTo`）。直接画不复权 K 线在信号之后拆股时价位线会错位，
  画前复权又连分红一起调。开发库实测 APH 2026-07-30 信号、09-03 拆股 1:2：折回后 09-03 收盘 164.14（原始 82.07），07-30 收盘 159.82 与信号价位一致。
- `KlineChart` 加可选 `markers` / `priceLines` / `zones`，不传时行为不变（行情页照常）。
- 验证中发现并修正：账本与信号列表逐条查信号 / 账本行（经隧道 9.7 秒 → 0.7 秒）；评估列表与审计把判定明细 jsonb 一并取回（列表改为不取明细列，3.8 秒 → 1.0 秒）；
  审计请求单独放宽到 30 秒超时；AI 分析时间按本地时间显示。
...
| 3 | 评估 / 信号 / 账本三张表、评估作业（美东 18:10）、边沿 / 冷却 / 过期、纸面账本、补偿、巡检第五段、接口 | 已完成（2026-09-17） |
| 4 | AI 否决：实测、输入与提示词与核验、V12 与接入评估作业；生产 `trader.env` 已配密钥，部署后核对 `ai.configured` | 已完成（2026-09-17） |
| 5 | 前端信号页、发布 3.0.0 | 已完成（2026-09-17） |

## 19. 3.0.1：首个四巫日暴露的问题与前端改造（2026-09-19 发布）

3.0.0 上线后第二个交易日（美东 2026-09-18，9 月第三个周五）的只读核对里，信号评估出了 51 条信号（前一天 1 条），
每日 AI 额度用完后 31 条没有模型结论，其中包括持仓 IBKR。设计 2026-09-19 经用户认可。

### 19.1 四巫日：只记录，不改判据

季度期权期货到期日成交量普遍放大，触发门的 RVOL ≥ 1.5 大面积成立。生产评估（515 只判定成功）逐门对比：

| | 09-17 | 09-18 |
| --- | --- | --- |
| RVOL 中位数 / 上四分位 | 1.00 / 1.21 | 2.18 / 2.79 |
| RVOL ≥ 1.5 | 52 | 462 |
| 趋势 / + 定位 | 232 / 88 | 237 / 102 |
| 触发门单独通过 | 18 | 189 |
| 四门全过 → 信号 | 1 | 51 |

成交量本身真实（AAPL 86M、平时约 37M；NVDA 189M、平时约 95M），不是数据错。`sentinel-v1` 的参数写死，
要排除这类日子须另出判据版本，由用户观察账本后再定。已知影响：同一天集中入场、彼此高度相关的一批样本会拉偏账本统计。

### 19.2 AI 额度按角色分配

评估作业边评估边调模型，每日上限（20 次，含手工）与作业内时长（15 分钟）先到先得，所以评估顺序就是额度的分配顺序。
3.0.0 的 `targets()` 先放全量成分股（`instrument` 表 `ORDER BY symbol`），池与持仓多半已在其中而保留原位——额度按字母发放，
09-18 在 GWW 用完，池里的 AMZN 因为 A 开头拿到了意见，持仓 IBKR 被预算跳过。

改为持仓 → 池 → 池外，同一角色内按代码；基准照旧排除。四门判定、边沿、冷却逐只独立，结果不受顺序影响；作业汇总计数不变。
没有改成"先全部评估、再挑候选统一排序分配额度"：那要拆开"是信号就与评估行同一事务写入"的结构，而池外按什么排序本身是个策略选择。
守护测试 `评估目标按持仓池池外排序且排除基准`（反证过：还原 3.0.0 顺序即失败）。

已知边界（不在本次改）：每日上限含手工调用，白天手工分析多了会挤占当晚作业；池上限 50，池本身大又逢四巫日时仅池成员就可能超过 20 次。

### 19.3 账本的 AI 分组按裁决而不是按有无分析

预算跳过、调用失败、拒答、截断、结构非法、输入构建失败都会写一行 `ai_analysis`（裁决 `ABSENT`）并挂到信号上。
3.0.0 的账本分组只看 `aiAnalysisId` 是否为空，把这些全算成 `ALLOW`：09-18 那批显示"放行 50 / 否决 2 / 无结论 0"，实际是 19 / 2 / 31，
"模型否决有没有用"的对比因此失真。改为按挂着的那次分析的 `verdict`：信号 `VETOED` → VETO，裁决 ALLOW → ALLOW，其余（含没挂分析）→ NONE。
新增 `AiAnalysisRepository.verdicts(ids)` 批量取裁决。分组是读时计算，部署后历史统计自动纠正，不需要迁移；
API.md 本来就写的是"NONE 没有模型结论"，接口不变。守护测试 `SignalFacadeLedgerTest`（反证过：还原只看 id 的写法即失败）。

没有采用的两个方案：用 `aiStance` 是否为空代替（现在碰巧等价，是隐含约定）；ABSENT 时不挂分析（丢掉"为什么没有意见"的线索，且已挂的要改库）。

### 19.4 信号详情抽屉的"手工分析"

3.0.0 只要信号挂着分析（哪怕是预算跳过）就展示它并隐藏"手工分析"按钮，被跳过的信号在界面上没法补一次意见。
改为：挂着的那次不是 OK 时，写明当晚没有结论的原因，再找同一标的、同一判定日最近一次 OK 的分析（手工补的）优先展示；
都没有时给按钮，旁边显示今天已调用 / 上限，额度用完置灰并提示美东 0 点重置。事后的手工意见只供参考，不改信号状态，也不改账本分组。

### 19.5 前端改造（第 1、2 期）

2026-09-18 浏览器实测五个页面后定案，分四期，3.0.1 带前两期：
- 第 1 期：品牌 T-Signal 与 TS 图标（`trader-web/scripts/make-favicon.py` 生成 ico）；去掉 1100px 上限改满宽，页头常驻环境徽标、当前作业、主题切换；
  暗色主题（跟随系统 + 手动覆盖，图表配色改读 CSS 变量）；公共层 `lib/format.ts`、`PageHeader`、`StatCard`、`useAutoRefresh`、`useTheme`、`stores/app.ts`；
  修 Element Plus 未配中文、不存在的路径白屏（加兜底路由，`SpaForwardControllerTest` 跳过参数路由并断言兜底存在）、`toISOString()` 取日跨日等。
  实测 1600px 下表格溢出归零（此前券商网关 +110px、财务报表 +242px）。
- 第 2 期：`/` 改仪表盘（今日信号、净值、跑批、网关、合并的审计问题、数据新鲜度、最近信号、净值走势；只复用既有接口），系统信息移到 `/system`，
  网关明细从展开行改抽屉。
- 剩第 3 期（行情页拆 tab）与第 4 期（基本面、账户、信号三页打磨）。

## 20. 3.0.2：实时账户（2026-09-19 发布）

仪表盘的账户区要实时的资金与盈亏，此前只有每日 18:00 的收盘快照。设计 2026-09-19 经用户认可：**净值用实时估算、订阅按需**。

### 20.1 实测（2026-09-19 美东周五盘后，只读探针，临时 client-id 41 / 42）

| 接口 | 实测 | 对设计的影响 |
| --- | --- | --- |
| `reqAccountSummary` 常驻 | 首批约 0.3 秒回齐；之后**逐条推、没有 End，约 3 分钟一批**（第 160 秒） | 资金可用，净值靠它只能到分钟级；逐条攒 300 ms 再发 |
| 账户汇总并发上限 | 同一客户端第 3 个报 **322**；另一个客户端此时照样能开 → **每客户端 2 个** | 常驻 1 个 + 每日快照的一次性 1 个 = 2，恰好不超；开发与生产各算各的 |
| `reqPnL` | **首条不完整**（只等于一只持仓的值），1 秒后第二条才完整；之后按变化约 5 秒一推，盘后照推（用盘后价） | 丢掉首条 |
| `reqPnLSingle` | 约 1.4 秒首推、按变化推；realized 为 Double.MAX_VALUE（未设）；value 即该仓市值 | 逐只市值秒级 |
| 加总 | 同一时刻 Σ 逐只当日 = 账户当日（946.95）、Σ 浮盈 = 账户浮盈（22,359.71）、Σ 市值 = 汇总股票市值（505,553.69），**精确相等** | 实时净值 = 现金 + 应计股息 + Σ 逐只市值；账户盈亏没到时可用逐只加总 |

`reqAccountUpdates` 仍不用：同一账户同时只允许一个客户端订阅，会与 ib-auto-trader 互踢（§16.1）。

### 20.2 结构

- **gateway-api** `LiveAccountGateway`（startLive / stopLive / liveActive）+ `LiveAccountListener`（onSummary / onPnl / onPositions / onPositionPnl / onLiveError）；
  domain 加 `AccountPnl`、`PositionPnl`。与 `AccountGateway` 的一次性查询分开，快照照旧用后者。
- **gateway-ibkr** `IbkrSubscriptions`：常驻订阅的 reqId → 处理器（注册表只管"请求 → 收齐 → 结束"），与注册表共用 reqId 计数器；
  回调在泵线程上只转交，处理器在 `ibkr-dispatch`（单线程）上按到达顺序执行。
  `IbkrLiveAccount`：一个账户的订阅状态机，只在 dispatch 线程上运行、不加锁——持仓 End 后按持仓逐只订 `reqPnLSingle`，
  清仓（数量 0）退订；账户汇总逐条攒批；首条账户盈亏丢弃。
  **重订**：首次连上、断线重连、1101 都走 `onConnected`。1101 时会话不变、旧订阅还挂在网关上，先取消再订，否则账户汇总撞上 322；
  断线重连是新会话，旧 reqId 已随会话失效，不再发取消。
- **core** `LiveAccountService`：按需——第一次读发起订阅（WARMING），**5 分钟没人读退订**（这个网关同时服务真正下单的 ib-auto-trader，能少占就少占）；
  推送只写 volatile 字段，读在请求线程。实时净值只对全是股票的账户估、缺任何一只的市值就不估（退回汇总净值）；
  账户盈亏没到时（首条被丢、价格静止时第二条迟迟不来）用逐只加总。金额保留两位小数（double 转过来带浮点尾巴）。
- **接口** `GET /api/account/live`，字段见 API.md。**前端**：仪表盘账户区与持仓表优先用实时（持仓表加"当日盈亏"金额列），
  净值走势从最后一份快照到当前实时净值画一段虚线；拿不到实时时退回收盘快照并写明。页面可见时 5 秒一轮（预热中 1.5 秒）。
- 不落库、不进快照，每日快照 / 对账 / 审计不变。

### 20.3 守护测试（均反证过）

`IbkrLiveAccountTest`：账户盈亏首条丢弃、逐只盈亏跟着持仓订退、同一会话重订先取消旧订阅、新会话不发旧取消、账户汇总攒批、未连接不留半截订阅、未设值映射成空。
`IbkrWrapperTest`：常驻订阅的错误（322）与推送转给订阅处理器。
`LiveAccountServiceTest`：第一次读发起订阅、实时净值按恒等式估算（探针同一时刻的数）、缺一只市值不估、账户盈亏没到时逐只加总、清仓后移除、闲置五分钟退订、网关未连不订阅。

### 20.4 当日盈亏的基准何时翻转（2026-09-20 ~ 09-21 实测，两段逐分钟采样）

原先的猜测是"午夜、盘前 04:00 或开盘 09:30 清零"，**三个都不对**。结论：**盈透在盘后时段结束（美东 20:00）之后翻转基准**，
中间有约 4 分钟空窗。生产上只读采样 `GET /api/account/live`，每分钟一条整包 JSON，两段合起来覆盖完整一天。

**第一段（09-20 周日 18:54 → 09-21 周一 10:30，934 条）**：整个上午基准都是**上一交易日收盘**，开盘处没有跳变。
SPY 的 `priorClose` 全程 761.690002（周五收盘），当日盈亏从 677 连续走到 2,896，开盘前后无台阶。
周一 17:43（收盘后 1 小时 43 分）基准仍是周五收盘，算术反证：(773.45 − 761.69) × 210 = 2,469.60，正是盈透给的 SPY 当日。
顺带证伪了"网关每日重启导致资金取不到"的猜测（见 §17 与 3.0.8）：03:45 重启、03:46 就恢复，`money` 到 10:30 都正常。

**第二段（09-21 周一 17:44 → 21:22，219 条）**：翻转落在 20:02 ~ 20:06。

| 美东 | 账户当日 | SPY `priorClose` | SPY 当日 | 逐只 `dailyPnl` |
| --- | --- | --- | --- | --- |
| 20:01 | 3,686.90 | null | 2,549.40 | 有值，对**周五**收盘 |
| 20:02 | 30.18 | null | null | 全部转 null |
| 20:03 ~ 20:05 | **0.00** | null | null | 全部 null，浮盈冻结在 25,662.73、`updatedAt` 停住 |
| 20:06 | 243.52 | **773.50** | 69.30 | 有值，对**周一**收盘 |
| 20:07 | 361.17 | 773.50 | 111.31 | 有值 |

20:06 与 20:07 两条的逐只当日合计与账户口径都差 0.00，口径自洽。

两条由此确定的边界：

- **20:02 ~ 20:05 约 4 分钟里 `pnl` 不是 null 但 `daily` 恒为 0.00**，逐只盈亏与 `priorClose` 同时为 null，浮盈不再更新。
  这是盈透的真实状态，不是故障，页面会照实显示"当日 0.00"；`IbkrLiveAccount.verifyPnl` 的逐只核对此时两边都是 0，照样通过。
- **当日盈亏用盈透自己的标价算，不是我们采到的 `last`**。20:06 那条里 BRK.B、GOOG、NVDA、IBKR 用 `(last − priorClose) × 数量` 精确对上，
  SPY（102.90 vs 69.30）与 TSLA（39.00 vs 6.00）对不上；到 20:07 这两只就对上了（111.30 vs 111.31、42.00 vs 42.00）——标价比行情最新价慢一拍。
  **别拿 `last` 去校验 `dailyPnl`**，要核对就用"逐只之和 vs 账户口径"（`verifyPnl` 正是这么做的）。

**第三段（采样续到 09-22 周二 10:00 自停，全程 976 条有效样本）已补齐最后一个证据：整段只有 20:06 这一个翻转点。**
SPY 的 `priorClose` 从 09-21 20:06 起恒为 773.50，穿过夜盘、凌晨、网关每日重启（03:45）、盘前 04:00、开盘 09:30，直到 09:59 都没有再变；
当日盈亏在开盘处连续（09:29 466.76 → 09:30 596.95 → 09:31 516.41），没有台阶；盘前它照常走负（04:01 −53.58、04:05 −141.42），
说明基准就是周一收盘、只是标的跌破了它。**"午夜 / 盘前 04:00 / 开盘 09:30 清零"三个猜测全部证伪，一天只翻转一次，在 20:00 盘后收市之后。**

同一份日志顺带记录的两件事：

- **网关每日重启只断 1 分钟且不伤数据**（3.0.8 之后）：03:45 一条 `DISCONNECTED`（"网关关闭了连接…4.7 秒后"），03:46 即回 LIVE；
  全程 `money` 一次都没空过、`lastError` 一条都没有、当日盈亏基准不受影响。976 条里 970 条 LIVE。
  注意这段**没有覆盖 3.0.8 真正修的那条路径**——采样每分钟读一次，实时账户始终不闲置，"闲置 5 分钟退订 → 再订"没有被触发过。
- 09-21 20:36 ~ 20:40 另有 5 分钟 `DISCONNECTED`（`[IBKR 502]` 建连失败，自行恢复）。断线期间 `money` 与盈亏保留断线前最后一次的值
  （251.01 连续 5 条不变），这是 §20.2 设计的行为，不是数据卡住。

### 20.5 3.0.3：只给盈透原值（2026-09-19 用户要求）

用户核对后要求账户数据全部取盈透提供的、与盈透 App 一致，不要本系统计算、以免产生误解。改动：
- **净值**改为盈透账户汇总的 NetLiquidation 原值，去掉"现金 + 应计股息 + Σ 逐只市值"的估算（代价：汇总约 3 分钟一推，可能比 App 晚）；股票市值同理取汇总。
- **当日 / 浮动盈亏**只用 reqPnL 原值，去掉"账户盈亏没到时用逐只加总"的兜底；没有有效值时为 null，页面显示"等待盈透推送"。
- **现价**改为订盈透行情 reqMktData 取 lastPrice（与 App「最新价」同源），不再用"市值 ÷ 数量"——探针实测两者不同（GOOG 最新价 346.08、逐只盈亏估值 21,353.42 ÷ 62 = 344.41）。
  账户有实时行情权限（marketDataType=1）；收盘后买卖价为 -1，只认 LAST（延迟行情下 68），延迟时标出；每只持仓占 1 条行情线，同样按需订、闲置退。
- **百分比**（当日 %、盈亏率、占净值）盈透接口不给，去掉；"较收盘快照"去掉。持仓表：代码 / 数量 / 成本价 / 最新价 / 市值 / 当日盈亏 / 浮动盈亏。
- 实时不可用时的收盘快照退回模式也不再叠加富途报价估算。
- **reqPnL 首条**再实测（同一会话退订再订）：首条仍不对（给出 991.93，正确是 915.99），约 1 秒后第二条才对；但生产 09-18 20:37 订上后 20 分钟没有第二条
  （盘后结束、价格静止）→ 加看门狗：10 秒没有有效值就退订再订，最多 3 次（重订时逐只盈亏已在跑，探针里第二条 1 秒就到）。
- 守护测试：`净值与现价都是盈透原值`、`账户盈亏没到时为空不加总`、`现价只取最新价`、`持仓行情跟着持仓订退`、`账户盈亏没有有效值时重订`（后两条之一与"不加总"均反证过）、`账户盈亏重订有上限`。

### 20.6 3.0.4：与盈透 App 逐项对照（2026-09-19）

用户给了盈透 App 里 GOOG 的截图，逐项对照 3.0.3 的取数：最新价 346.08（行情 lastPrice）、Position 62、Market Value $21,353（逐只盈亏 value 21,353.42）、
Average Price 346.81、Today 45.26、Unrealized P&L -148.58、Realized —，**全部一致**。

- **不切换 reqAccountUpdates**（ib-auto-trader 已废弃、用户曾让只给生产用）：探针实测它首批与账户汇总同值，之后只按整 3 分钟推 updateAccountTime，不比汇总快；
  而且它的持仓市值（updatePortfolio 21,458.82、市价 346.11）**与 App 对不上**。同一时刻 GOOG 有三个"价格"：行情 346.08、updatePortfolio 346.11、逐只盈亏估值 344.41，
  App 用的是行情价与逐只盈亏的市值。
- App 界面另有三项派生数，用户要求加回，按 App 同口径在后端算好：涨跌（+2.40 / +0.70%，前收取行情 CLOSE tick = 343.68，与 App PRIOR CLOSE 同值）、
  Cost Basis（21,502 = 346.81 × 62）、% of Portfolio（4.20% = 市值 ÷ 净值）。守护测试 `派生项与盈透App截图一致` 用截图的数（反证过：占组合分母换成股票市值即失败）。
- 价格显示两位小数，与 App 一致（盈透行情给到 346.082）。
- **reqPnL 的"首条"规则推翻**（3.0.3 一律丢首条，开发实例重启后当日盈亏一直"等待盈透推送"，看门狗重订 3 次都没用）。再实测：逐只盈亏稳定运行后再订，
  reqPnL **只推一条而且是对的**（915.99），价格静止时不再有第二条——丢首条等于把唯一的正确值扔了。三种情况合看，推送对不对与"第几条"无关，
  但对的推送与逐只当日 / 浮盈之和都精确相等、错的差几十到上千 → 改为**用逐只之和核对**（容差 1 美元）：对得上才采用，采用过一次后续直接放行；
  逐只没到齐的先挂起、到齐再核；10 秒没有通过核对的仍重订。显示的始终是 reqPnL 原值，逐只之和只用来判断完整与否。
  守护测试 `唯一一条正确推送直接采用`（反证：恢复丢首条即失败）、`与逐只之和对不上的账户盈亏不采用`（反证：去掉核对即失败）、`账户盈亏先到时等逐只到齐再核`。

### 20.7 3.0.9：账户汇总改为常驻订阅（2026-09-23 生产实测，并订正 3.0.8 的定性）

**现象**：09-23 上午实时账户反复卡在 WARMING、`money` 取不到，`lastError` 是 `[IBKR 322]`，
3.0.8 加的三次自动重订全被拒。跑批未受影响——当天 06:00 的账户快照作业 #80 是 OK 的。

**定性（时间线逐格吻合）**：盈透「每客户端 2 个账户汇总订阅」的上限算的是**订过的次数**，
不是当前活着的订阅——`cancelAccountSummary` 发出去了，名额也不释放，只有客户端重连才清零。

| 时刻（北京） | 动作 | 占用 |
| --- | --- | --- |
| 09-22 22:04 | 实时账户闲置退订（已发取消） | 1 |
| 09-23 06:00 | 快照作业订 → 取数 → 发取消 | 2（作业本身成功，它是第 2 个） |
| 09-23 06:53 | 实时账户再订 | 第 3 个 → **322** |
| 08:12 / 11:09 | 再订、各重试 3 次 | 仍是第 3 个 → 全败 |

**3.0.8 的判断因此是错的**：当时把"部署后恢复正常"当成补发取消生效，实际上是**部署重启清空了名额**。
补发取消只记得实时账户自己上一次的 reqId，取消不掉快照作业留下的那一个；
重试 3 次 × 3 秒对一个根本不会释放的名额也毫无意义。这条教训写进了验证纪律：
**症状在重启后消失，不能算修复验证通过。**

**改法**：把"订了又退"这个动作整个消灭掉。账户汇总改为 `IbkrAccountSummaryFeed`——
整个连接周期只订一次的常驻订阅，随连接建立、随断开释放，实时账户与每日快照共用：

- `startLive` / `stopLive` 与闲置退订都不再影响它，闲置只摘监听器（退订也不还名额，退它没有意义）。
- 快照作业不再自己发请求，`AccountGateway.accountSummary` 改读常驻订阅的最新值。
- 挂上监听器时若已有数据立刻补一条 → 闲置后再打开页面，资金立刻就有，不用等下一批（约 3 分钟）。

**代价与边界**：
- 快照的资金口径从"当场查"变成"最近一次推送"（≤3 分钟）。收盘后的估值基准受此影响可忽略，但口径变了要记住。
- 陈旧度阈值 10 分钟、等待上限 90 秒：等不到就抛异常，快照记 FAILED，由 21:00 补偿重试——**不拿旧值糊弄**。
- **这是规避不是修复**：盈透为什么不释放名额仍然不知道。
- **1101 数据丢失后的重订仍会再占一个名额**，那一次若被 322 只能重连。本版不加自动重连（会把持仓、
  盈亏、行情的订阅一起断掉），留作实测观察点。

**怎样才算验证通过**（吸取 3.0.8 的教训，部署后正常不作数）：必须跨过一次每日 03:45 网关重启、
一次每日快照作业、至少一次"闲置退订 → 再读"，并连续 2 个交易日不出现 322。

## 21. 第 5 期：分部估值 SOTP（3.1.0）

把一家公司按业务线拆开估值。模型是使用者提出的：每条业务线 **量 × 价 = 营收 → × 净利率 = 净利 → × 本益比 = 业务价值**；
`目标年股价 =（Σ 业务价值 + 目标年净现金）÷ 目标年股数`，再按要求回报率**折回基准日**才能和现价比。

**这是估值工具，不是预测器，也不联动入场信号与纸面账本。** 券商只给合并报表，没有分部量价、没有一致预期，
所以每条业务线的量、价、净利率、本益比都得使用者自己填。系统负责三件事：自动带入公司级底座、做算术、
**把自相矛盾的假设指出来**。

### 21.1 七处修正落成结构性约束

2026-09-19 评估这个模型时列了 7 处必须补的地方。它们不是提示文字，是代码里的硬约束：

| # | 修正 | 落地方式 |
| --- | --- | --- |
| 1 | 必须折现 | `SotpAssumptions.discountFactor()`，**没有开关**；结果里「折回今天」单独成列并注明只有这列能和现价比 |
| 2 | 利润率用净利率 | 字段名写死 `netMargin`，并给**口径核对**：Σ业务净利 ÷ Σ业务营收 vs 最近一期实际合并净利率，偏离 > 10 个百分点标红 |
| 3 | 防重复计算 | 每条业务线 `scopeNote` **必填**，空白直接 400；模板预置「FSD 只算订阅」「Robotaxi 只算打车」 |
| 4 | 分业务 PE 最敏感 | 给参考区间（汽车 10~20、能源 15~30、软件 20~40）但允许覆盖；**单因子敏感度**只动一条、其余保持基准，按摆幅倒序 |
| 5 | 净现金用目标年值 | 与当前值分成两格，目标年**不默认等于**当前；界面给「用当前值」按钮但要手动点 |
| 6 | 股数预测到目标年 | 同上分两格 |
| 7 | 期权型业务用情景 | 三情景（BEAR / BASE / BULL）**缺一不可**，`SotpSegment` 构造时校验，不接受单点估值 |

另有**反推**：不问「值多少」，问「现价已经押注了什么」——按要求回报率算出目标年需达到的股价、市值，
按本益比基准倒推所需净利与所需年化增长。**缺本益比基准时整个返回 null，不硬编倍数**（硬编只会把猜测伪装成计算）。

### 21.2 底座取数（`SotpBasisService`）

全部从库里取，不用手填。三处按已知的券商行为写，不是想当然：

- **净现金** = 资产负债表 `8003 现金及短投 − 8058 短期借款 − 8069 长期借款`；**融资租赁（8060 + 8070）单列、默认不计入**。
  TSLA 2026/Q2 实测 43,524 − 1,340 − 7,721 = **34,463（百万美元）**，与 09-19 手工算的 345 亿一致。
- **TTM 营收与归母净利**只认期别带 `/Q` 的四个季报——**年报与四季报期末是同一天**，按期末去重会把年报算进去。
  TSLA 实测 TTM 营收 1036.19 亿 ÷ 净利 38.04 亿 = 3.67%，与券商独立给的 `14005 归母净利率` 0.036731 互相印证。
- **本益比基准**取近 5 年日 K 市盈率**正值**的中位数：券商在亏损期与基金上给 0，**0 是无数据不是真值**，混进分位统计会把中位数拉低。
- **适用性**：取不到财报 → NOT_APPLICABLE（基金与指数没有分部利润可拆）；亏损、缺本益比基准、金融业（该用 PB+ROE）、
  房地产 REITs（该用 FFO）→ CAUTION 并逐条给原因。**判断「该不该有财报」看有没有取到，不看 `secType`**——
  券商把美股 REITs 也归为 Trust，按类型过滤会误杀 25 只。

**股数三种口径全给、系统不替使用者选**：券商流通股、市值 ÷ 现价、归母净利 ÷ 稀释每股收益。
TSLA 实测分别是 28.19 亿 / 39.79 亿 / 35.22 亿，**三个数互不相同**，选哪个直接改变目标股价，这个选择必须由使用者做。

### 21.3 存储与接口

`sotp_model`（V13）一张表，业务线连同三情景整体存 `segments` jsonb：这套东西永远整体存整体取，
不需要跨标的按字段查；项目已有先例 `signal_evaluation.detail`。按（标的, 名称）唯一，一只标的可存多套。

五条接口见 [API.md](API.md#分部估值-sotp第-5-期)。试算走两段路径 `calc/{symbol}`，避免 `POST .../sotp/calc` 被当成 `symbol=calc`
（`ValuationControllerTest` 守住）。服务层收**平铺的请求对象**再显式构造领域对象——让 Jackson 去撞领域记录的校验，
报错会被包成反序列化异常、丢掉中文原因。

### 21.4 已知边界

- **数据缺口无法回避**：没有分部量价与一致预期，工具帮你把假设算清楚、挡住自相矛盾的假设，但**不能替你预测**。
- 回测式的验证做不了：分部假设是对未来的判断，没有历史样本可回放。口径核对、敏感度、反推是仅有的三道自洽性检查。
