...
| 3 | 账户与持仓（**进行中，§16**）：盈透账户资金与收盘持仓的每日快照与对账、持仓自动维护池里的 HOLDING 角色 |
...
5. **账户号**：库里只存哈希与脱敏形式，明文只在服务端内存；多账户时按 `trader.ibkr.account` 选。
...
7. **HOLDING 自动维护**：快照后、盈透连上时、手工试跑时触发。新买入标 HOLDING（原为 POOL 的记下）；清仓后原为 POOL 的回 POOL，
   自动加入的移出池（数据保留）。请求不完整、或返回空而上次非空时不改；HOLDING 不再接受手工添加。
